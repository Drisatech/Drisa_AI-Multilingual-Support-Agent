...
// --- Temporary Avatar Generation Logic ---
async function generateNewAvatar() {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey || apiKey === 'MY_GEMINI_API_KEY') {
    console.error('[Server] Cannot generate avatar: GEMINI_API_KEY is missing');
    return;
  }

  const ai = new GoogleGenAI({ apiKey });
  const prompt = "A high-quality, professional portrait of a handsome, black male customer care agent on duty. He is wearing a professional headset with a microphone, dressed in corporate attire (suit and tie), and wearing stylish black-framed eyeglasses with subtle light reflections. He is smiling warmly and looks helpful. Professional office background, soft lighting, 4k resolution.";

  try {
    console.log('[Server] Generating new agent avatar...');
    ...
    for (const p of paths) {
      ...
      console.log(`[Server] Saved avatar to ${p}`);
    }
    console.log('[Server] Avatar generation successful');
  } catch (error) {
    console.error('[Server] Avatar generation failed:', error);
  }
}

// Trigger generation on startup if the file is missing or if we want to force it
// generateNewAvatar(); 

app.get('/api/admin/generate-avatar', async (req, res) => {
  await generateNewAvatar();
  res.send('Avatar generation triggered. Check server logs.');
});
